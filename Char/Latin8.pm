...
######################################################################
#
# Char::Latin8 - Source code filter to escape Latin-8 script
#
# http://search.cpan.org/dist/Char-Char::Latin8/
#
...
BEGIN { CORE::require Char::Elatin8; }
...
my $function_ord;         # ord()   to ord()   or Char::Latin8::ord()
my $function_ord_;        # ord     to ord     or Char::Latin8::ord_
my $function_reverse;     # reverse to reverse or Char::Latin8::reverse
my $function_getc;        # getc    to getc    or Char::Latin8::getc
...
END
    ...
    print Char::Latin8::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Elatin8::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Latin8::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Elatin8::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Latin8::escape_script {
    ...
    # read Latin-8 script
    my $fh = gensym();
    Char::Elatin8::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Elatin8(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Elatin8 %s;\n", $Char::Latin8::VERSION); # require run-time routines version

        # use Char::Latin8 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Latin8 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Latin8::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Latin8.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Latin8.pm $version, but @{[__FILE__]} is version $Char::Latin8::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Elatin8.pm $version, but \\\$Char::Elatin8::VERSION is \$Char::Elatin8::VERSION" if \$Char::Elatin8::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Latin8::VERSION) {
                    die "Script $0 required Char/Latin8.pm $version, but @{[__FILE__]} is only version $Char::Latin8::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Latin8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Latin8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Latin8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Latin8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Char::Elatin8
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Latin8::re_t=%s; while($Char::Latin8::re_t =~ %s){%s local $^W=0; local $Char::Latin8::re_r=%s; %s$Char::Latin8::re_t="$`$Char::Latin8::re_r$'"; pos($Char::Latin8::re_t)=length "$`$Char::Latin8::re_r"; } return $Char::Latin8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin8::re_r=eval $Char::Latin8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Latin8::re_r=%s; %s"%s$Char::Latin8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Latin8::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin8::re_r=%s; %s%s="$`$Char::Latin8::re_r$'"; pos(%s)=length "$`$Char::Latin8::re_r"; $Char::Latin8::re_n++} return %s$Char::Latin8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Latin8::re_r=%s; %s%s="%s$Char::Latin8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Latin8::re_r=%s; %s%s="%s$Char::Latin8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin8::re_r=eval $Char::Latin8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Latin8.pm and Char/Elatin8.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Latin8;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Latin8;
  use Char::Latin8 ver.sion;             --- require minimum version
  use Char::Latin8 ver.sion.0;           --- expects version (match or die)

  # "no Char::Latin8;" not supported

  or

  $ perl Char/Latin8.pm Latin-8_script.pl > Escaped_script.pl.e
...
Char::Latin8 software is "middleware" between perl interpreter and your Perl script
written in Latin-8.
...
By "use Char::Latin8;", it automatically interpret your script as Latin-8. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Latin8.pm, Char/Elatin8.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Elatin8.pm
works well.
...
=head1 Software Composition
   ...
   Char/Latin8.pm               --- source code filter to escape Latin-8
   Char/Elatin8.pm              --- run-time routines for Char/Latin8.pm
...
You need write 'use Char::Latin8;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Latin8;
  ---------------------------------

=head1 Calling 'Char::Elatin8::ignorecase()' (Char/Latin8.pm provides)

Char/Latin8.pm applies calling 'Char::Elatin8::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Elatin8::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Elatin8.pm provides)
...
Definitions in Char/Elatin8.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Latin8.pm provides)

Char/Latin8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Latin8.pm and Char/Elatin8.pm provide)

Insert 'Char::Elatin8::' at head of function name. Char/Elatin8.pm provides your script Char::Elatin8::*
functions.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Elatin8::lc        Character
  lcfirst     Char::Elatin8::lcfirst   Character
  uc          Char::Elatin8::uc        Character
  ucfirst     Char::Elatin8::ucfirst   Character
  fc          Char::Elatin8::fc        Character
  chr         Char::Elatin8::chr       Character
  glob        Char::Elatin8::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Functions (Char/Latin8.pm provides)

Char/Latin8.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Elatin8.pm provides)

Char/Elatin8.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Latin8' }" at head.
Store the standard module modified for Char::Latin8 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Latin8 and change
'use utf8;' to 'use Char::Latin8;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Elatin8.pm provides these
  functions.
...
Char::Latin8 software remains one language and one interpreter.
