...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin8::uc($char[$i]) ne Char::Elatin8::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin8::uc($char[$i]) . Char::Elatin8::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin8/Char::Latin8/JA.pm
...
Definitions in Char/Elatin8.pm.
  ...
  @{Char::Elatin8::eV}             qr{(?:[^\x0C\x0A\x0D])}
