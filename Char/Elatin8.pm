...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Elatin8::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Elatin8::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Latin-8 $string for delimiters that match pattern and splits the Latin-8
  ...
  pattern, so the delimiters may be of any size and need not be the same Latin-8
  $string on every match. If the pattern doesn't match at all, Char::Elatin8::split returns
  the original Latin-8 $string as a single substring. If it matches once, you get
  ...
  If Latin-8 $string is omitted, the function splits the $_ Latin-8 string.
...
=item Chop string
  ...
  Chops off the last character of a Latin-8 string contained in the variable (or
  Latin-8 strings in each element of a @list) and returns the character chopped.
  The Char::Elatin8::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Elatin8::index($string,$substr,$position);
  $pos = Char::Elatin8::index($string,$substr);

  Returns the position of the first occurrence of $substr in Latin-8 $string.
  The start, if specified, specifies the $position to start looking in the Latin-8
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Elatin8::index function returns -1.

=item Reverse index string

  $pos = Char::Elatin8::rindex($string,$substr,$position);
  $pos = Char::Elatin8::rindex($string,$substr);

  Works just like Char::Elatin8::index except that it returns the position of the last
  occurence of $substr in Latin-8 $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Latin-8 string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Latin-8 string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Latin-8 string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Latin-8 string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Elatin8::chr(65) is "A" in either ASCII or Latin-8, not Unicode,
  and Char::Elatin8::chr(0x82a0) is a Latin-8 HIRAGANA LETTER A. For the reverse of
  Char::Elatin8::chr, use Char::Latin8::ord.
