...
BEGIN {
    ...
    sub Char::Elatin8::binmode(*;$);
    sub Char::Elatin8::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Elatin8::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Elatin8::open;
    }
}
...
=item Make character
  ...
  set. For example, Char::Elatin8::chr(65) is "A" in either ASCII or Latin-8, and
  Char::Elatin8::chr(0x82a0) is a Latin-8 HIRAGANA LETTER A. For the reverse of Char::Elatin8::chr,
  use Char::Latin8::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Elatin8::binmode(FILEHANDLE, $disciplines);
  Char::Elatin8::binmode(FILEHANDLE);
  Char::Elatin8::binmode($filehandle, $disciplines);
  Char::Elatin8::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, Char::Latin8 software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Elatin8::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Elatin8::open(FILEHANDLE, $expr);
  $rc = Char::Elatin8::open(FILEHANDLE);
  $rc = Char::Elatin8::open(my $filehandle, $mode, $expr);
  $rc = Char::Elatin8::open(my $filehandle, $expr);
  $rc = Char::Elatin8::open(my $filehandle);

  * autovivification filehandle
  * three arguments

  If you are using perl5.005, Char::Latin8 software emulate perl5.6's open function.
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Elatin8::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Elatin8::open my $fh, ">logfile" or die ...
  ...
  Char::Elatin8::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Elatin8::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Elatin8::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Elatin8::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Elatin8::open(FH, "<:crlf:encoding(Char::Latin8)", $path) or die "can't open $path: $!";
