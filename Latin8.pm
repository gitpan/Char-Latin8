...
my $function_ord;         # ord()   to ord()   or Latin8::ord()
my $function_ord_;        # ord     to ord     or Latin8::ord_
my $function_reverse;     # reverse to reverse or Latin8::reverse
my $function_getc;        # getc    to getc    or Latin8::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin8::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Latin8::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Latin8::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Latin8::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Latin8::re_t=%s; while($Latin8::re_t =~ %s){%s local $^W=0; local $Latin8::re_r=%s; %s$Latin8::re_t="$`$Latin8::re_r$'"; pos($Latin8::re_t)=length "$`$Latin8::re_r"; } return $Latin8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin8::re_r=eval $Latin8::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Latin8::re_r=%s; %s"%s$Latin8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Latin8::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin8::re_r=%s; %s%s="$`$Latin8::re_r$'"; pos(%s)=length "$`$Latin8::re_r"; $Latin8::re_n++} return %s$Latin8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Latin8::re_r=%s; %s%s="%s$Latin8::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Latin8::re_r=%s; %s%s="%s$Latin8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin8::re_r=eval $Latin8::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Elatin8.pm
works well.
...
=head1 Calling 'Elatin8::ignorecase()' (Latin8.pm provides)

Latin8.pm applies calling 'Elatin8::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Latin8.pm provides)

Latin8.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Latin8.pm and Elatin8.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (Latin8.pm provides)

Latin8.pm removes 'bytes::' at head of subroutine name.
...
'use utf8;' to 'use Latin8;' in its. You need help yourself for now.
