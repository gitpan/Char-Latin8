...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin8::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin8::open;
    }
}
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin8::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin8::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin8::split returns
...
=item Chop string
  ...
  The Elatin8::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin8::index($string,$substr,$position);
  $pos = Elatin8::index($string,$substr);
  ...
  the Elatin8::index function returns -1.

=item Reverse index string

  $pos = Elatin8::rindex($string,$substr,$position);
  $pos = Elatin8::rindex($string,$substr);

  Works just like Elatin8::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin8::chr(65) is "A" in either ASCII or Latin-8, and
  Elatin8::chr(0x82a0) is a Latin-8 HIRAGANA LETTER A. For the reverse of Elatin8::chr,
  use Latin8::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin8::binmode(FILEHANDLE, $disciplines);
  Elatin8::binmode(FILEHANDLE);
  Elatin8::binmode($filehandle, $disciplines);
  Elatin8::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, Latin8 software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin8::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin8::open(FILEHANDLE, $expr);
  $rc = Elatin8::open(FILEHANDLE);
  $rc = Elatin8::open(my $filehandle, $mode, $expr);
  $rc = Elatin8::open(my $filehandle, $expr);
  $rc = Elatin8::open(my $filehandle);

  * autovivification filehandle
  * three arguments

  If you are using perl5.005, Latin8 software emulate perl5.6's open function.
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin8::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin8::open my $fh, ">logfile" or die ...
  ...
  Elatin8::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin8::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin8::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin8::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin8::open(FH, "<:crlf:encoding(Latin8)", $path) or die "can't open $path: $!";
