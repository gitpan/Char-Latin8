...
######################################################################
#
# Elatin8 - Run-time routines for Latin8.pm
#
...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Elatin8::index($name, '::') == -1) && (Elatin8::index($name, "'") == -1)) {
            my $pkg;
...
sub Elatin8::rindex($$;$);
sub Elatin8::lcfirst(@);
sub Elatin8::lcfirst_();
sub Elatin8::lc(@);
sub Elatin8::lc_();
sub Elatin8::ucfirst(@);
sub Elatin8::ucfirst_();
sub Elatin8::uc(@);
sub Elatin8::uc_();
sub Elatin8::fc(@);
sub Elatin8::fc_();
sub Elatin8::ignorecase;
sub Elatin8::classic_character_class;
sub Elatin8::capture;
sub Elatin8::chr(;$);
sub Elatin8::chr_();
sub Elatin8::glob($);
...
# avoid: Name "Elatin8::foo" used only once: possible typo at here.
${Elatin8::dot}         = ${Elatin8::dot};
${Elatin8::dot_s}       = ${Elatin8::dot_s};
${Elatin8::eD}          = ${Elatin8::eD};
${Elatin8::eS}          = ${Elatin8::eS};
${Elatin8::eW}          = ${Elatin8::eW};
${Elatin8::eH}          = ${Elatin8::eH};
${Elatin8::eV}          = ${Elatin8::eV};
${Elatin8::eR}          = ${Elatin8::eR};
${Elatin8::eN}          = ${Elatin8::eN};
${Elatin8::not_alnum}   = ${Elatin8::not_alnum};
${Elatin8::not_alpha}   = ${Elatin8::not_alpha};
${Elatin8::not_ascii}   = ${Elatin8::not_ascii};
${Elatin8::not_blank}   = ${Elatin8::not_blank};
${Elatin8::not_cntrl}   = ${Elatin8::not_cntrl};
${Elatin8::not_digit}   = ${Elatin8::not_digit};
${Elatin8::not_graph}   = ${Elatin8::not_graph};
${Elatin8::not_lower}   = ${Elatin8::not_lower};
${Elatin8::not_lower_i} = ${Elatin8::not_lower_i};
${Elatin8::not_print}   = ${Elatin8::not_print};
${Elatin8::not_punct}   = ${Elatin8::not_punct};
${Elatin8::not_space}   = ${Elatin8::not_space};
${Elatin8::not_upper}   = ${Elatin8::not_upper};
${Elatin8::not_upper_i} = ${Elatin8::not_upper_i};
${Elatin8::not_word}    = ${Elatin8::not_word};
${Elatin8::not_xdigit}  = ${Elatin8::not_xdigit};
${Elatin8::eb}          = ${Elatin8::eb};
${Elatin8::eB}          = ${Elatin8::eB};
...
sub Elatin8::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin8::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin8::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin8::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Elatin8::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Elatin8::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Elatin8::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Elatin8::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Elatin8::uc($char[$i]);
                my $fc = Elatin8::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Elatin8::uc($char);
                        my $fc = Elatin8::fc($char);
                        if ($uc eq $fc) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $fc = Elatin8::fc($char)) ne $char) {
                $pattern .= $fc;
            }
            else {
        ...
        my $matchsub = sub { Elatin8::fc($_[0]) =~ /\A $pattern \z/xms };
...
INNER:
        for my $leaf (@leaf) {
            ...
            if (Elatin8::index($leaf,'.') == -1 and   # if name does not have a dot in it *and*
                CORE::length($leaf) <= 8 and        # name is shorter than or equal to 8 chars *and*
                Elatin8::index($pattern,'\\.') != -1  # pattern has a dot.
            ) {
                if (&$matchsub("$leaf.")) {
                    push @matched, "$head$leaf";
                    next INNER;
...
sub Latin8::getc(;*@) {
    ...
    croak 'Too many arguments for Latin8::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Elatin8::dot_s} \z/oxms) {
...
sub Latin8::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Elatin8::index($_[0], $_[1], CORE::length(Latin8::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Elatin8::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Latin8::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Latin8::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Elatin8::rindex($_[0], $_[1], CORE::length(Latin8::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Elatin8::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Latin8::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Latin8::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin8::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin8::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin8::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin8::MATCH()};
    }
...
# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin8::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Elatin8::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Elatin8::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Elatin8::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Elatin8::glob';
    }
...
# subroutines of package Elatin8
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                  }
    elsif (/\G \b Latin8::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Latin8::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Latin8::escape'; }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';            }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin8::chop';       }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';             }
    elsif (/\G \b Latin8::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Latin8::index';       }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin8::index';      }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';            }
    elsif (/\G \b Latin8::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Latin8::rindex';      }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Elatin8::rindex';     }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::lc';         }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::lcfirst';    }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::uc';         }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::ucfirst';    }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::fc';         }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Elatin8::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Elatin8::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin8::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Elatin8::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Elatin8::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Elatin8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Elatin8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Elatin8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Elatin8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Elatin8::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Elatin8::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Elatin8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Elatin8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Elatin8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Elatin8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Elatin8::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Elatin8::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Elatin8::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Elatin8::PREMATCH()]}
            ...
            $e_string .= q{Elatin8::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Elatin8::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Elatin8::MATCH()};
            $slash = 'div';
        }
...
# subroutines of package Elatin8
        ...
        elsif ($string =~ /\G \b Latin8::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin8::eval \b                                /oxgc) { $e_string .= 'eval Latin8::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Elatin8::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin8::index \b                               /oxgc) { $e_string .= 'Latin8::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Elatin8::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Latin8::rindex \b                              /oxgc) { $e_string .= 'Latin8::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Elatin8::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Elatin8::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Elatin8::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Elatin8::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Elatin8::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Elatin8::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Elatin8::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Elatin8::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Elatin8::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Elatin8::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Elatin8::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Elatin8::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Elatin8::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Elatin8::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Elatin8::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Elatin8::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Elatin8::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Elatin8::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Elatin8::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Elatin8::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Elatin8::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Elatin8::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Elatin8::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Elatin8::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin8::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin8::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Elatin8::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Elatin8::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Elatin8::uc($char[$i]) ne Elatin8::fc($char[$i]))) {
            if (CORE::length(Elatin8::fc($char[$i])) == 1) {
                $char[$i] = '['   . Elatin8::uc($char[$i])       . Elatin8::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Elatin8::uc($char[$i]) . '|' . Elatin8::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin8::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Elatin8::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin8::ignorecase(Elatin8::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin8::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Elatin8::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin8::ignorecase(Elatin8::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin8::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Elatin8::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Elatin8::ignorecase(Elatin8::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Elatin8::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Latin8::re_t=%s; while($Latin8::re_t =~ %s){%s local $^W=0; local $Latin8::re_r=%s; %s$Latin8::re_t="$`$Latin8::re_r$'"; pos($Latin8::re_t)=length "$`$Latin8::re_r"; } return $Latin8::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin8::re_r=CORE::eval $Latin8::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Latin8::re_r=%s; %s"%s$Latin8::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Latin8::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin8::re_r=%s; %s%s="$`$Latin8::re_r$'"; pos(%s)=length "$`$Latin8::re_r"; $Latin8::re_n++} return %s$Latin8::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Latin8::re_r=%s; %s%s="%s$Latin8::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Latin8::re_r=%s; %s%s="%s$Latin8::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin8::re_r=CORE::eval $Latin8::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Elatin8::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Elatin8::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
This Elatin8 module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Elatin8::split(/pattern/,$string,$limit);
  @split = Elatin8::split(/pattern/,$string);
  @split = Elatin8::split(/pattern/);
  @split = Elatin8::split('',$string,$limit);
  @split = Elatin8::split('',$string);
  @split = Elatin8::split('');
  @split = Elatin8::split();
  @split = Elatin8::split;
  ...
  at all, Elatin8::split returns the original string as a single substring, If it
  ...
  @chars  = Elatin8::split(//,  $word);
  @fields = Elatin8::split(/:/, $line);
  @words  = Elatin8::split(" ", $paragraph);
  @lines  = Elatin8::split(/^/, $buffer);
  ...
  print join(":" => Elatin8::split(/ */, "hi there"));
  ...
  Elatin8::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Elatin8::split(/:/, $_, 3);
  ...
  splits on whitespace just as Elatin8::split with no arguments does. Thus,
  Elatin8::split(" ") can be used to emulate awk's default behavior. In contrast,
  Elatin8::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Elatin8::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Elatin8::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Elatin8::split()) {
          $count{$word}++;
      }
  }

  The inverse of Elatin8::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Elatin8::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Elatin8::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Elatin8::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Elatin8::chop(@list);
  $chop = Elatin8::chop();
  $chop = Elatin8::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Elatin8::chop subroutine is used primary to remove the newline
  ...
  since Elatin8::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Elatin8::chop a literal, only a variable. If you Elatin8::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Elatin8::chop(@lines);

  You can Elatin8::chop anything that is an lvalue, including an assignment:

  Elatin8::chop($cwd = `pwd`);
  Elatin8::chop($answer = <STDIN>);

  This is different from:

  $answer = Elatin8::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Elatin8::chop returns the character
  ...
  In the most general case, Elatin8::chop can be expressed using substr:

  $last_code = Elatin8::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Elatin8::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Elatin8::chop;
...
=item Index string
  ...
  $byte_pos = Elatin8::index($string,$substr,$byte_offset);
  $byte_pos = Elatin8::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Elatin8::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Elatin8::rindex($string,$substr,$byte_offset);
  $byte_pos = Elatin8::rindex($string,$substr);

  This subroutine works just like Elatin8::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Elatin8::index).
  ...
  while (($byte_pos = Latin8::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Elatin8::fc subroutine for case-insensitive comparisons via Latin8
  software.

=item Lower case first character of string

  $lcfirst = Elatin8::lcfirst($string);
  $lcfirst = Elatin8::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Elatin8::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Elatin8::ucfirst($string);
  $ucfirst = Elatin8::ucfirst_;
  ...
  $titlecase = Elatin8::ucfirst(substr($word,0,1)) . Elatin8::lc(substr($word,1));
  ...
  $do_not_use = Elatin8::ucfirst(Elatin8::lc($word));
...
=item Fold case string
  ...
  $fc = Elatin8::fc($string);
  $fc = Elatin8::fc_;

  New to Latin8 software, this subroutine returns the full Unicode-like casefold of
  ...
  Elatin8::fc($a) eq Elatin8::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Latin8 software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Elatin8::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Elatin8::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Elatin8::chr($code);
  $chr = Elatin8::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Elatin8::chr(65) is "A" in either
  ASCII or Latin-8, not Unicode. For the reverse of Elatin8::chr, use Latin8::ord.

=item Filename expansion (globbing)

  @glob = Elatin8::glob($string);
  @glob = Elatin8::glob_;
  ...
  The Elatin8::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Elatin8::glob('"*e f*"');
  @spacies = Elatin8::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Elatin8::glob("'*${var}e f*'");
  @spacies = Elatin8::glob(qq("*${var}e f*"));
